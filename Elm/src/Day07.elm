...
part1 s =
    let
        allBags : Dict String (List String)
        allBags =
            s |> parse |> containerLookup
        ...
        contains bags =
            if bags == [] then
                ...
                let
                    containers =
                        bags
                            |> List.concatMap (\b -> Dict.get b allBags |> Maybe.withDefault [])
                            |> List.unique
                            |> List.filterNot (\b -> List.member b bags)
                in
                bags
                    |> List.append (contains containers)
                    |> List.unique
    in
    contains [ "shiny gold" ] |> List.count ((/=) "shiny gold")


part2 : String -> Int
part2 s =
    let
        allBags : Dict String (List ( Int, String ))
        allBags =
            s |> parse |> Dict.fromList
...
parse : String -> List ( String, List ( Int, String ) )
parse =
    String.lines >> List.map (clean >> parseBag)
...
parseBag string =
    case splitAndTrim "contain" string of
        container :: [ "no other" ] ->
            ( container, [] )
...
splitAndTrim : String -> String -> List String
splitAndTrim x =
    String.split x >> List.map String.trim


quantityAndBagName : String -> ( Int, String )
quantityAndBagName s =
    case String.toList s of
        cq :: cn ->
            let
                q =
                    cq |> String.fromChar |> String.toInt |> Maybe.withDefault 0

                name =
                    cn |> String.fromList |> String.trim
            in
            ( q, name )
...
containerLookup : List ( String, List ( Int, String ) ) -> Dict String (List String)
containerLookup =
    List.map (Tuple.mapSecond (List.map Tuple.second))
        >> List.concatMap (\( x, y ) -> y |> List.map (\z -> ( z, x )))
        >> List.foldl
            (\( x, y ) dict -> Dict.insert x (y :: (Dict.get x dict |> Maybe.withDefault [])) dict)
            Dict.empty
