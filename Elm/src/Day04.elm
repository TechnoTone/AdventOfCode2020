...
part2 string =
    string
        |> parseInput
        |> List.count isValidPassport


isValidPassport : Passport -> Bool
isValidPassport passport =
    True
        && (passport.byr |> isBetween ( 1920, 2002 ))
        && (passport.iyr |> isBetween ( 2010, 2020 ))
        && (passport.eyr |> isBetween ( 2020, 2030 ))
...
parseMaybeString key =
    P.oneOf
        [ P.map Just (parseString key)
        , P.map (\_ -> Nothing) P.spaces
        ]
...
parseEyeColor key =
    P.succeed identity
        ...
        |. P.symbol ":"
        |= P.oneOf
            [ P.succeed Amb |. P.keyword "amb"
            , P.succeed Blu |. P.keyword "blu"
            , P.succeed Brn |. P.keyword "brn"
            , P.succeed Gry |. P.keyword "gry"
            , P.succeed Grn |. P.keyword "grn"
            , P.succeed Hzl |. P.keyword "hzl"
            , P.succeed Oth |. P.keyword "oth"
            ]
...
parseHairColor key =
    P.succeed identity
        ...
        |. P.symbol ":"
        |. P.symbol "#"
        |= (P.chompWhile isValidChar |> P.getChompedString)


isValidChar : Char -> Bool
isValidChar c =
    List.member c validChars
